...
#define EE_ADDR &eeprom_read_byte*/
...
const uint8_t CH_AETR[] =
    {AILERON, ELEVATOR, THROTTLE, RUDDER, CH5, CH6, CH7, CH8, CH9, CH10, CH11, CH12, CH13, CH14, CH15, CH16};

const uint8_t CH_TAER[] =
    {THROTTLE, AILERON, ELEVATOR, RUDDER, CH5, CH6, CH7, CH8, CH9, CH10, CH11, CH12, CH13, CH14, CH15, CH16};

const uint8_t CH_RETA[] =
    {RUDDER, ELEVATOR, THROTTLE, AILERON, CH5, CH6, CH7, CH8, CH9, CH10, CH11, CH12, CH13, CH14, CH15, CH16};

const uint8_t CH_EATR[] =
    {ELEVATOR, AILERON, THROTTLE, RUDDER, CH5, CH6, CH7, CH8, CH9, CH10, CH11, CH12, CH13, CH14, CH15, CH16};
...
//	#endif
    ...
    MProtocol_id_master = random_id(10, 0);
...
{
    int returnvalue = (val + JOYSTICK_MAX) * (CHANNEL_MAX_100 - CHANNEL_MIN_100) / (JOYSTICK_MAX * 2) + CHANNEL_MIN_100;
    return returnvalue;
}
...
{
    if (r->tv_nsec >= NSEC_PER_SEC || r->tv_nsec <= -NSEC_PER_SEC) {
        long tv_nsec = r->tv_nsec;
        r->tv_sec = r->tv_sec + r->tv_nsec / NSEC_PER_SEC;
        r->tv_nsec = tv_nsec % NSEC_PER_SEC;
    }
...
int main(int argc, char **argv)
{
   ...
   // sched_setscheduler(0, SCHED_FIFO, &priority);
    ...
    struct channel_spec channel_specs[] =
        {{"/dev/input/js0", JS_EVENT_AXIS, 0, 0}, // roll
         {"/dev/input/js0", JS_EVENT_AXIS, 1, 1}, // pitch
         {"/dev/input/js1", JS_EVENT_AXIS, 3, 1}, // throttle
         {"/dev/input/js2", JS_EVENT_AXIS, 2, 0}, // yaw
         {"/dev/input/js1", JS_EVENT_AXIS, 4, 0}, // arm
        };
    init_joystick(channel_specs, 5);
    BIND_IN_PROGRESS;

    protocol = PROTO_FRSKYD;

    int result = 0x40 & 0x3F;
    printf("result: 0x%02X\r\n", result);

    protocol_init();


    if (clock_gettime(CLOCK_REALTIME, &gettime_now) != 0) {
        printf("could not get clock_time");
        exit(1);
    }

    struct timespec next_callback_time = gettime_now;


    while (1) {
        next_callback_time.tv_nsec += remote_callback() * 1000;
        timespec_normalize(&next_callback_time);

        do {
            int16_t *joystick_channels = update_channels();

            int count = MIN(NUM_CHN, JOYSTICK_CHANNEL_COUNT);

            for (int i = 0; i < count; i++) {
                Channel_data[i] = convert_channel_to_ppm(joystick_channels[i]);
            }
        ...
        while (is_timespec_a_less_than_b(&gettime_now, &next_callback_time));

        //printf("timenow  %ld\r\n", gettime_now.tv_nsec);
        //for (uint8_t i = 0; i < NUM_CHN; i++)
        //	Channel_data[i] = channelValue++;
...
if(mode_select==MODE_SERIAL && IS_RX_FLAG_on)		// Serial mode and something has been received
{
    ...
    if (IS_CHANGE_PROTOCOL_FLAG_on) { // Protocol needs to be changed or relaunched for bind
...
static void update_led_status(void)
{
    ...
    if (blink < millis()) {
        ...
        else if (remote_callback == 0) { // Invalid protocol
...
{
    uint8_t bank=0; //eeprom_read_byte((EE_ADDR)EEPROM_BANK_OFFSET);
    if(bank>=NBR_BANKS)
...
static void protocol_init()
{
    ...
    if (next_callback > 32000) { // next_callback should not be more than 32767 so we will wait here...
...
#endif
    if ((rx_ok_buff[0] != cur_protocol[0]) || ((rx_ok_buff[1] & 0x5F) != (cur_protocol[1] & 0x5F))
        || ((rx_ok_buff[2] & 0x7F) != (cur_protocol[2] & 0x7F))) { // New model has been selected
        ...
        if ((rx_ok_buff[1] & 0x80) != 0 || IS_AUTOBIND_FLAG_on)
            ...
            BIND_DONE;
        protocol = (rx_ok_buff[0] == 0x55 ? 0 : 32)
            + (rx_ok_buff[1] & 0x1F);    //protocol no (0-63) bits 4-6 of buff[1] and bit 0 of buf[0]
...
/*#if not defined (ORANGE_TX) && not defined (STM32_BOARD)
	...
	static uint32_t random(int var)
	{
	 random();
	}
